...
function loadTourList(plannerId) {
    console.log("[입력] 플래너 ID:", plannerId);

    // 서버에서 데이터 가져오기
    fetch(`/api/planner-spot/list?planner_id=${plannerId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
        ...
        .then(data => {
            console.log("[출력] 서버로부터 받은 데이터:", data);
            renderTourList(data);
        })
        .catch(error => {
            console.error("[오류] 데이터를 불러오지 못했습니다:", error);
        });
}
...
function renderTourList(data) {
    const tourList = document.getElementById("tourList");
    tourList.innerHTML = ""; // 기존 리스트 초기화

    data.forEach((item, index) => {
        const listItem = document.createElement("li");
        const textSpan = document.createElement("span");
        textSpan.textContent = `${index + 1}. ${item.spotName}`;
        textSpan.className = "tour-item";
        ...
        editBtn.className = "edit-btn";
        editBtn.onclick = () => {
            const newName = prompt("새로운 이름을 입력하세요:", item.spotName);
            if (newName) {
                console.log("관광지 이름 변경:", item.spotName, "->", newName);
                textSpan.textContent = `${index + 1}. ${newName}`;
                // 서버 업데이트 로직 필요
            }
        };
