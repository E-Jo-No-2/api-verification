...
console.log("URL에서 가져온 위도:", latitude);
console.log("URL에서 가져온 경도:", longitude);
...
function loadTourSpots(longitude, latitude) {
    console.log(`주변 관광지 데이터를 불러옵니다. 위도: ${latitude}, 경도: ${longitude}`);
    fetch(`/api/tour/nearby?longitude=${longitude}&latitude=${latitude}`)
        .then(response => {
            console.log("서버로부터의 응답을 받았습니다:", response);
...
cards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        fetch(`/api/landmark/coordinates?landmarkName=${selectedLandmarkName}`)
            ...
            .catch(error => console.error("좌표를 불러오는 중 오류 발생:", error));
    });
});
...
function transformLocations(data) {
    ...
    Object.keys(data).forEach(theme => {
        ...
        if (Array.isArray(themeData)) {
            ...
            console.warn(`배열을 기대했지만 ${typeof themeData}을(를) 받음: ${theme}`);
        }
    });
...
function addToTourList(locationName) {
    ...
    editBtn.onclick = () => {
        ...
        if (newName) {
            console.log("관광지 이름이 변경되었습니다:", locationName, "->", newName);
...
function selectLocation(name) {
    ...
    console.log("장소 저장 데이터 전송 중:", dataToSend);
    ...
    fetch('/api/tour/save', {
        ...
        .then(response => {
            if (response.ok) {
                console.log("장소가 성공적으로 저장되었습니다:", name);
